...
function generate_sys_resource_usage_report(){
        ...
        format_output_align_columns true "ANPs," $totalANPs>>/tmp/final-summary.csv
        ...
        python3 get-sys-usage.py -q 'ovnkube_controller_num_egress_firewall_rules' -s $JOB_START -e $JOB_END -t getInfo| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`  
        format_output_align_columns true "EgressFirewallRules," $maxValue>>/tmp/final-summary.csv

        python3 get-sys-usage.py -q 'sum(kube_pod_status_phase{}) by (phase)' -s $JOB_START -e $JOB_END -t getInfo| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`  

        python3 get-sys-usage.py -q 'count(kube_secret_info{})' -s $JOB_START -e $JOB_END -t getInfo| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "Secret," $maxValue>>/tmp/final-summary.csv

        python3 get-sys-usage.py -q 'count(kube_configmap_info{})' -s $JOB_START -e $JOB_END -t getInfo| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "ConfigMap," $maxValue>>/tmp/final-summary.csv
       
        python3 get-sys-usage.py -q 'count(kube_service_info{})' -s $JOB_START -e $JOB_END -t getInfo| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "Service," $maxValue>>/tmp/final-summary.csv

        python3 get-sys-usage.py -q 'count(openshift_route_info{})' -s $JOB_START -e $JOB_END -t getInfo| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "Route," $maxValue>>/tmp/final-summary.csv

        awk 'BEGIN{for(c=0;c<80;c++) printf "-"; printf "\n"}'

        echo "Query metric from prometheus and generate performance report ..."
        awk 'BEGIN{for(c=0;c<80;c++) printf "-"; printf "\n"}'

        curl -X GET ${ES_SERVER}/_cat/indices?format=json | jq 'length'
        curl -X PUT -H "Content-Type: application/json" ${ES_SERVER}/${ES_INDEX}/_settings -d '{"index.mapping.total_fields.limit":20000}'
        echo

        format_output_align_columns false "DB Size" "Max Value" "Avg Value">>/tmp/final-summary.csv
        python3 get-sys-usage.py -q 'ovn_db_db_size_bytes{db_name=~"OVN_Northbound"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo  
        fi

      
        python3 get-sys-usage.py -q 'ovn_db_db_size_bytes{db_name=~"OVN_Northbound"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`                
        format_output_align_columns true "nbdb," $maxValue $avgValue>>/tmp/final-summary.csv
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        fi
        echo

        python3 get-sys-usage.py -q 'ovn_db_db_size_bytes{db_name=~"OVN_Southbound"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'ovn_db_db_size_bytes{db_name=~"OVN_Southbound"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi                
        format_output_align_columns true "sbdb," $maxValue $avgValue>>/tmp/final-summary.csv

        format_output_align_columns false "RAM Size" "Max Value" "Avg Value">>/tmp/final-summary.csv
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"nbdb"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`           
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"nbdb"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`      
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        ...
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"sbdb"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`          
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"sbdb"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`          
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        ...
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"northd"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`       
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"northd"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`       
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi        
        format_output_align_columns true "northd," $maxValue $avgValue>>/tmp/final-summary.csv 

        python3 get-sys-usage.py -q 'container_memory_rss{container=~"ovn-controller"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`      
        if [[ $IF_SAVE_TO_ES == "true" ]];then      
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"ovn-controller"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`      
        if [[ $IF_SAVE_TO_ES == "true" ]];then      
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi        
        format_output_align_columns true "ovn-controller," $maxValue $avgValue>>/tmp/final-summary.csv            

        python3 get-sys-usage.py -q 'container_memory_rss{container=~"ovnkube-controller"}' -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"ovnkube-controller"}' -s $JOB_START -e $JOB_END -t topAvgOverTime| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi        
        format_output_align_columns true "ovnkube-controller," $maxValue $avgValue>>/tmp/final-summary.csv        

        python3 get-sys-usage.py -q 'container_memory_rss{container=~"kube-rbac-proxy-node|kube-rbac-proxy-ovn-metrics|nbdb|northd|ovn-acl-logging|ovn-controller|ovnkube-controller|sbdb"}' -s $JOB_START -e $JOB_END -t topMaxOverTimeSumByPod| tee metric_result.txt
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        python3 get-sys-usage.py -q 'container_memory_rss{container=~"kube-rbac-proxy-node|kube-rbac-proxy-ovn-metrics|nbdb|northd|ovn-acl-logging|ovn-controller|ovnkube-controller|sbdb"}' -s $JOB_START -e $JOB_END -t topAvgOverTimeSumByPod| tee metric_result.txt
        avgValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi        
        format_output_align_columns true "ovnkube-node-pod," $maxValue $avgValue>>/tmp/final-summary.csv 

        format_output_align_columns false "Latency" "Value">>/tmp/final-summary.csv
        python3 get-ovn-metrics.py -q "ovnkube_controller_ready_duration_seconds" -s $JOB_START -e $JOB_END -t topMaxOverTime| tee metric_result.txt
        ...
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "ovnkube_controller_ready_duration_seconds," "$maxValue">>/tmp/final-summary.csv

        IntValue=$(echo "$maxValue" | cut -d. -f1)
        if [ $IntValue -gt 50 ];then
            echo "The max value of ovnkube_controller_ready_duration_seconds is great than expected value"
            exit 1
        fi
      
        python3 get-ovn-metrics.py -q "ovnkube_node_ready_duration_seconds" -s $JOB_START -e $JOB_END  -t topMaxOverTime| tee metric_result.txt
        if [[ $IF_SAVE_TO_ES == "true" ]];then 
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        ...
        python3 get-ovn-metrics.py -q "ovnkube_controller_sync_duration_seconds" -s $JOB_START -e $JOB_END  -t topMaxOverTime| tee metric_result.txt
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "ovnkube_controller_sync_duration_seconds," "$maxValue">>/tmp/final-summary.csv
        ...
        python3 get-ovn-metrics.py -q "ovnkube_controller_pod_event_latency_seconds_bucket" -s $JOB_START -e $JOB_END  -t histogramQuantile| tee metric_result.txt
        if [[ $IF_SAVE_TO_ES == "true" ]];then
        curl -k -sS -X POST -H "Content-type: application/json"  -H "Cache-Control:no-cache" ${ES_SERVER}/${ES_INDEX}/_doc -d @ovn-metric-es-payload.json
        echo
        fi
        maxValue=`cat metric_result.txt |grep -w No.1| awk '{print  $NF}'| tr -d ' '`
        format_output_align_columns true "ovnkube_controller_pod_event_latency_seconds_bucket," "$maxValue">>/tmp/final-summary.csv

        IntValue=$(echo "$maxValue" | cut -d. -f1)
        if [ $IntValue -gt 10 ];then
            echo "The max value of ovnkube_controller_pod_event_latency_seconds_bucket is great than expected value"
            exit 1
        fi
  
        awk 'BEGIN{for(c=0;c<120;c++) printf "-"; printf "\n"}'
        echo "The final summary report of large scale anp/netpol/egressfirewall"
        awk 'BEGIN{for(c=0;c<120;c++) printf "-"; printf "\n"}'
