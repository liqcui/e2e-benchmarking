...
def get_ovn_metrics(metricName, start_time, end_time, promQLOperation):
        ...
        if promQLOperation == "topMaxOverTime":
            ...
            promQL = "histogram_quantile(0.9, sum by(pod, event, le) (rate({}[{}])))".format(metricName,timeDuration)
        else:
        ...
        for r in results:
            ...
            if "ovnkube_controller_sync_duration_seconds" in promQL:
              ...
              metricRow=("No."+str(i)+" "+metricName,podName+":"+resourceName,str(metricValue))
              ...
              metricRow=("No."+str(i)+" "+metricName,podName+":"+metricEvent,str(metricValue))
            else:
