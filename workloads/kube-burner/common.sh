...
function check_traffic_between_anp_zones(){
    ...
    TARGET_NS=`oc get ns |grep -w $TARGET_NS | awk '{print $1}'|head -20`
    if [[ -z $TARGET_NS ]];then
...
function restartOVNPODs(){
      ...
      OVN_NODE_POD_NAMES=`oc -n openshift-ovn-kubernetes  get pods |grep -v -i -E 'ovnkube-control-plane|NAME' | awk '{print $1}'| tail -15`
...
function run_large_networkpolicy_egressfirewall_anp_workload(){
       ...
       if [[ $IF_MASTER_CARD_CASE == "true" ]];then     
             ANP_PODS="anp-cidr-pods anp-open-pods anp-test-pods"
       else
