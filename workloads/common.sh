...
function create_sdn_egressfirewall(){

for NAMESPACE in `oc get ns | grep cluster-density-v2| awk '{print $1}'`
do
...
spec:
  ...
  - type: Allow
    to:
      cidrSelector: 110.110.110.110/24
  - type: Allow
    to:
      cidrSelector: 118.118.118.118/24                     
  - type: Allow
    to:
      dnsName: www.google.com          
  - type: Deny
    to:
      cidrSelector: 8.8.8.4/24      
  - type: Deny
    to:
...
function live-migration-post-check(){
    ...
    echo "Sleep 15 minutes after ovn live migration..."
    sleep 900
                
    export TEST_STEP="Sleep 15 minutes after OVN CNI migration - second boot"
    export CREATE_TIME=`date +"%y-%m-%d %H:%M:%S.%N" -d "+8 hours"`                   
    export QUERY_TIME=`date +"%y-%m-%d %H:%M:%S.%N" -d "+8 hours"` 
    get_ovn_node_system_usage_info
    ...
    echo "wait for 30 minutes before reboot nodes"
    sleep 1800
    export TEST_STEP="Before reboot nodes when OVN CNI migration complete"
    export CREATE_TIME=`date +"%y-%m-%d %H:%M:%S.%N" -d "+8 hours"`                   
    export QUERY_TIME=`date +"%y-%m-%d %H:%M:%S.%N" -d "+8 hours"` 
    get_ovn_node_system_usage_info 
